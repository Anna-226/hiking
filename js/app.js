(() => {
    ...
    if (parallax) {
        ...
        let threshHoldSets = [];
        for (let i = 0; i <= 1; i += .005) threshHoldSets.push(i);
    ...
    const defaultOffset = window.innerHeight;
    console.log(defaultOffset);
    let lastScroll = 0;
    const scrollPosition = () => window.pageYOffset;
    const containHide = () => headerBody.classList.contains(`hide`);
    window.addEventListener("scroll", (() => {
        if (scrollPosition() > lastScroll && !containHide() && scrollPosition() > defaultOffset) headerBody.classList.add(`hide`); else if (scrollPosition() < lastScroll && containHide()) headerBody.classList.remove(`hide`);
        lastScroll = scrollPosition();
        if (scrollPosition() > defaultOffset) headerBody.classList.add(`black`); else headerBody.classList.remove(`black`);
    }));
    window["FLS"] = true;
    isWebp();
    menuInit();
